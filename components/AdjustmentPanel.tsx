    ...
    <div className="flex flex-col h-full bg-transparent overflow-hidden relative">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar relative z-10">
          <div className="mb-6">
              ...
              <div className="group border border-white/5 bg-black rounded-none overflow-hidden focus-within:border-highlight transition-all relative"> {/* Changed border-adjust to border-highlight */}
                  <div className="flex justify-between items-center bg-zinc-900/40 px-3 py-1.5 border-b border-white/5">
                      <span className="text-[7px] font-mono text-zinc-600 uppercase tracking-widest font-black group-focus-within:text-highlight transition-colors">LUT_MODIFIER</span> {/* Changed text-adjust to text-highlight */}
                      <button onClick={() => setIsSaveModalOpen(true)} disabled={!userPrompt.trim()} className="text-zinc-600 hover:text-highlight transition-colors"> {/* Changed hover:text-adjust to hover:text-highlight */}
          ...
          <div className="bg-white/[0.03] p-3 border border-white/5 mb-6">
                <div className="flex justify-between mb-3 items-center">
                    ...
                    <span className="text-sm font-display text-highlight italic tracking-widest">{intensity}%</span> {/* Changed text-adjust to text-highlight */}
                </div>
                <input 
                    type="range" 
                    min="10" max="100" 
                    ...
                    className="w-full h-1 bg-white/10 appearance-none cursor-pointer accent-highlight" // Changed accent-adjust to accent-highlight
                />
          </div>
          ...
          <div>
            ...
            <div className="bg-black/40 border border-zinc-800 p-4 rounded-sm shadow-inner">
                <div className="preset-grid pb-4"> {/* Removed pb-20 from here, added pb-4 to the grid itself */}
                    {allPresets.map(preset => (
                        <button
                            ...
                            className={`preset-card ${
                                ...
                                ? 'bg-highlight border-highlight text-black z-10' // Changed bg-adjust/border-adjust to bg-highlight/border-highlight
      ...
      <div className="p-4 border-t border-white/5 bg-zinc-950/60 shrink-0 relative z-10 backdrop-blur-md">
          <button onClick={handleApply} disabled={isLoading || (!selectedPreset && !userPrompt.trim())} className="execute-btn group hover:border-highlight transition-colors"> {/* Changed hover:border-adjust to hover:border-highlight */}
              <span className={`font-black italic uppercase tracking-[0.2em] text-[10px] transition-colors ${isLoading ? 'text-zinc-500' : 'text-zinc-500 group-hover:text-highlight'}`}> {/* Changed group-hover:text-adjust to group-hover:text-highlight */}
                  {isLoading ? 'Calibrating...' : 'Execute Grade'}
              </span>
              <SlidersIcon className={`w-3.5 h-3.5 transition-colors ${isLoading ? 'text-zinc-500' : 'text-zinc-500 group-hover:text-highlight'}`} /> {/* Changed group-hover:text-adjust to group-hover:text-highlight */}
          </button>
      </div>
    </div>
  );
